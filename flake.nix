{
  ...
  outputs = { self, nixpkgs, ... }@inputs:
    let
      ...
      packageName = "zigbee-commander";

      # Haskell package overrides:
      packageOverrides = haskell: {
        serialport = haskell.serialport.overrideAttrs (_orig: {
          src = ./vendor/serialport;
          meta.broken = false;
        });
        zigbee-znet25 = haskell.zigbee-znet25.overrideAttrs (_orig: {
          src = ./vendor/zigbee-znet25;
          doCheck = false; # FIXME: enocding/decoding tests are failing
          meta.broken = false;
        });
      };
